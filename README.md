...
# Surevey results(probability using sets)
...
def: the probability of an event occuring given that another event has already occured
...
P(Likes| Female) = N(Likes and Female)/N(Female)
...
known as how many arragement occured in set of elements out of total

ex:

this is known as when there is objecs A B C : those are labled, out of it, how many arragement made when it sorted in queu however there is count with diferent odering sequences.
...
ex: 10 runners are comit to maraton so how many changes in sequnce has in unique first 3 winners out of total

P(10,3) = 10!/7! = 8*9*10 = 720

ex:  there is a lock with 3 digit numbers to unlock so how possibile to open at 1 run FYI: for any digit comprice of 60 combinations
...
# combinations
 ...
 in the situation where there is an unique features in the set of object : number of items with unique features; however, there order in sorted qeueu arragement doesnt matters like purmutation,taken out in the stance known as combination
...
in group of 10 people how many of 3 people's group could be created 
...
ex:
for the market value of product price in account of mean is 125 Rs at deviation of 5.8 Rs what is probability of product valued in 155 Rs
...
using integration the pdf into pdf(0) to pdf(upper bound: end) as a fraction of from pdf(z=155) to pdf(upper bound: end) given the propability  
...
### 6 geometric probability distribution
...
ex: rolls two dies to come up double at the given probability come up in first sucess(k)
...
ex: rolls two dies to come up double at the given probability come up in second sucess(k)
...
ex: rolls two dies to come up double at the given probability come up in 3rd sucess(k)
...
ex: rolls two dies to come up double at the given probability come up in 4th sucess(k)
